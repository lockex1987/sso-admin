                ...
                <form @submit.prevent="submitForm()" novalidate>
                    <div class="modal-header">
                        <h4 class="modal-title">
                            Ứng dụng của người dùng {{user.username}}
                        </h4>
                    ...
                    <div class="modal-body text-body">
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th class="text-center">Mã ứng dụng</th>
                                    <th class="text-center">Tên ứng dụng</th>
                                    <th class="text-center">Gán ứng dụng</th>
                                </tr>
                            </thead>

                            <tbody>
                                <tr v-for="r in filteredAppList" :key="r.id">
                                    <td>
                                        {{r.code}}
                                    </td>
                                    <td>
...
export default {
    ...
    methods: {
        ...
        async submitForm() {
            ...
            const params = {
                id: this.user.id,
                apps: this.appList.filter(r => r.isGranted).map(r => r.id)
            };

            const { data } = await axios.post('/user/apps', params);
         ...
         */
        async getAppList() {
            const { data } = await axios.get('/app/all');
            data.forEach(e => {
                e.isGranted = false;
            });
            this.appList = data;
        ...
        async getAppsOfUser() {
            ...
            const { data } = await axios.get('/user/apps', { params });
